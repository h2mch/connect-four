            ...
            while (row < height && column < width) {
                ...
                if (score == 3 * DISC_QUANTIFIER) {
                    ...
                    if ((row < height - 1) && (column < width - 1) && isNextDiscInColumn(row + 1, column + 1)) {
                        decisionBase.add(new Result(ScoreDiagnonalBackward.class, column + 1, 3 * DISC_QUANTIFIER, disc));
                    }
                    // check upper left
                    if ((row > 2) && (column > 2) && isNextDiscInColumn(row - 3, column - 3)) {
                        decisionBase.add(new Result(ScoreDiagnonalBackward.class, column - 3, 3 * DISC_QUANTIFIER, disc));
                    }
                    score = 0;
                }
                if (score > 0) {
                    ...
                    if ((row < height - 2) && (column < width - 2) && isNextDiscInColumn(row + 1, column + 1)) {
                        decisionBase.add(new Result(ScoreDiagnonalBackward.class, column + 1, score, disc));
                        if (typedBoard.get(row + 2).get(column + 2) == disc) {
                            decisionBase.add(new Result(ScoreDiagnonalBackward.class, column + 1, score + DISC_QUANTIFIER, disc));
                        }
                    }
                    //check upper left
                    if ((row > 2) && (column > 2) && isNextDiscInColumn(row - 2, column - 2)) {
                        decisionBase.add(new Result(ScoreDiagnonalBackward.class, column - 2, score, disc));
                        if (typedBoard.get(row - 3).get(column - 3) == disc) {
                            decisionBase.add(new Result(ScoreDiagnonalBackward.class, column - 2, score + DISC_QUANTIFIER, disc));
                        }
                    }
                }
